...
class ClassroomResultController extends Controller
{
    ...
    public function show(Result $result_slug)
    {
        ...
        $result = Result::whereId($result_slug->id)->first();
        ...
        $number_of_subject = ResultDetail::whereResultId($result_slug->id)->count();
        ...
        $countAttendance = StudentAttendance::whereStudentId($result_slug->student_id)->whereYearId($year->id)->whereAttendanceStatus(1)->count();
        ...
        return view('teacher.classroom.result-show', compact('result_slug', 'results', 'student', 'year', 'attendance_per_cent', 'result_average_score', 'result_total_score', 'grade', 'classroom', 'x','result_average_score_total', 'session_average', 'result'));
    ...
    public function update_result(Request $request)
    {
        $data = request()->validate([
            ...
            'average_score' => ['required', 'string'],
            'grand_score' => ['required', 'string'],
        ]);

        Result::whereId($request->result_id)->update([
            'grand_score' => $request->grand_score,
            'average_score' => $request->average_score,
            'session_average' => $request->session_average,
            'grade' => $request->grade,
            'comment' => $request->comment,
            'attendance_per_cent' => $request->attendance,
            'result_status' => 1,
        ]);

        return redirect('/teacher/classroom-result')->with('success', 'Data saved successfully');
    }
